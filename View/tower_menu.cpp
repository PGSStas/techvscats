...
TowerMenu::TowerMenu(
    int creation_time, std::shared_ptr<const Building> tower,
    std::vector<std::shared_ptr<TowerMenuOption>> options) :
    creation_time_(creation_time), tower_(std::move(tower)),
...
void TowerMenu::Draw(QPainter* painter,
  ...
  for (size_t i = 0; i < options_.size(); i++) {
    ...
    options_[i]->Draw(painter,
                      size_handler.GameToWindowCoordinate(GetCoordinate(i,
                                                                        button_size)),
                      window_button_size);
  }
